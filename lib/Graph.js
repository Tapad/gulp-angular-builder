...
Graph.prototype.build = function () {
	...
	var globalDependencies = this.options.globalDependencies,
		...
		fullBuild = this.options.fullBuild;
	...
	Object.keys(nodes).forEach(function (path) {
		...
		Object.keys(resolves).forEach(function (item) {
			...
			resolvesMap[nodeMap[item].path] = (resolvesMap[nodeMap[item].path] || []).concat(resolves[item]);
		});
	});
	...
	Object.keys(nodes).forEach(function (path) {
		...
		if (node.metadata.type.required || fullBuild) {
			addRequired(path);
		}
	});
	...
	function addRequired(key, optional, parent) {
		...
		var selfResolves = node.metadata.resolves.__self || [];
		var deps = node.metadata.dependencies;

		deps.component.forEach(function (component) {
			if ((resolvesMap[key] && !!~resolvesMap[key].indexOf(component)) || (resolvesMap[node.path] && !!~resolvesMap[node.path].indexOf(component)) || !!~selfResolves.indexOf(component)) {
				return;
			}
		...
		deps.filterToken.forEach(function (filter) {
			addRequired("filter:" + filter, true, node);
		});
...
function pathMatches(p, check, every) {
	if (check == null) {
		return false;
	}
...
function getPriority(priorityList, p) {
	p = p.replace(new RegExp("\\" + path.sep, "g"), "/");
	...
	return l - priorityList.reduce(function (priority, item, i) {
		...
		return !!p.match(item) ? i : priority;
	}, l);
}
