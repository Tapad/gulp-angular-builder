...
function parseContent(contents, metadata) {
		...
		onopentag: function (name, attributes) {
			...
			Object.keys(attributes).forEach(function (attr) {
				if (attr == "ng-app") {
					...
					while (attrValue.match(filterRegex) !== null) {
						attrValue = attrValue.replace(filterRegex, function (match, f) {
							filterToken.push(f);
							return "expression";
						});
					}
	...
	filterToken.forEach(function (dep) {
		metadata.addDependency("filterToken", dep);
	});
