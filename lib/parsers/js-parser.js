...
function handleResolve(node, metadata) {
	...
	if (node.parent.type == "MemberExpression" && node.parent.parent && node.parent.parent.type == "AssignmentExpression" && node.parent.parent.right.type == "ObjectExpression") {
		...
		obj.properties.forEach(function (property) {
			...
			if(!!~controller.indexOf(" as ")) {
				controller = controller.split(" as ")[0];
			}
			return false;
		});
	} else {
...
function handleDecorator(node, metadata) {
	...
	var componentName = node.parent.parent.arguments[0].value;
	if (componentName.indexOf("Directive") === componentName.length - 9) {
		metadata.addDependency("directiveToken", componentName.replace(/Directive$/, ""));
	} else {
		metadata.addDependency("component", componentName);
	}
